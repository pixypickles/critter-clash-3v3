...
let progress={unlocked:0,titles:[],unlockedSkills:[],pendingBoss:null,defeatedBosses:[],specialChampions:[],dojoDefeated:[],beastUnlocked:[],beastDefeated:[]};
function loadSavedTeam(){try{let d=JSON.parse(localStorage.getItem(SAVE_KEY)||'null');if(d){if(Array.isArray(d.formation)&&d.formation.length===3)formation=d.formation.map(v=>TYPES[v]?v:'sword');if(Array.isArray(d.tactics)&&d.tactics.length===3)formationTactics=d.tactics.map(v=>TACTICS[v]?v:'balanced');formationSkills=formation.map((v,i)=>d.skills?.[i]&&SKILLS[d.skills[i]]?d.skills[i]:TYPES[v].defaultSkill);formationSkillsB=formation.map((v,i)=>d.skillsB?.[i]&&SKILLS[d.skillsB[i]]?d.skillsB[i]:'none')}}catch(e){}try{let d=JSON.parse(localStorage.getItem(PROGRESS_KEY)||'null');if(d&&Number.isInteger(d.unlocked))progress={unlocked:Math.max(0,Math.min(RANKS.length-1,d.unlocked)),titles:Array.isArray(d.titles)?d.titles:[],unlockedSkills:Array.isArray(d.unlockedSkills)?d.unlockedSkills:[],pendingBoss:Number.isInteger(d.pendingBoss)?d.pendingBoss:null,defeatedBosses:Array.isArray(d.defeatedBosses)?d.defeatedBosses:[],specialChampions:Array.isArray(d.specialChampions)?d.specialChampions:[],dojoDefeated:Array.isArray(d.dojoDefeated)?d.dojoDefeated:[],beastUnlocked:Array.isArray(d.beastUnlocked)?d.beastUnlocked:[],beastDefeated:Array.isArray(d.beastDefeated)?d.beastDefeated:[]}}catch(e){}}
...
function restartBossBattle(){if(!bossBattle)return;if(bossBattle.source==='dojo')startDojoBoss(bossBattle.dojo);else if(bossBattle.source==='beast'){let b=BEAST_BOSSES[bossBattle.beast];beginBoss(b,{source:'beast',beast:bossBattle.beast,replay:bossBattle.replay});}else startBoss()}
