...
const TYPES={sword:{name:'剣＋盾',r:'sword',l:'shield',speed:180},spear:{name:'両手槍',r:'spear',l:'spearGuard',speed:165},dagger:{name:'短剣二刀流',r:'daggerR',l:'daggerL',speed:210},doubleShield:{name:'双盾',r:'dualShield',l:'dualShield',speed:130}};
...
function unit(team,i,type){
  ...
  return {team,i,type,x:bx,y:ys[i],r:27,alive:true,face:team?Math.PI:0,cd:0,stun:0,shield:false,shieldA:0,spearGuard:0,spearGuardCd:0,ai:team===1||i>0,species:(i%3),handAnimL:0,handAnimR:0,attackPose:null}
}
...
function label(v){return ({sword:'剣',spear:'突き',spearGuard:'回転防御',shield:'盾',daggerR:'右短剣',daggerL:'左短剣',dualShield:'両盾'})[v]||v}
...
function hand(a,side,down=true){
  ...
  let e=nearestEnemy(a);if(e)a.face=angle(a,e);
  if(side==='l')a.handAnimL=kind.startsWith('dagger')?.24:.42;else a.handAnimR=kind.startsWith('dagger')?.24:.42;
  attack(a,kind,side);
  // 短剣は高速、剣は見てから反応できるよう遅め、槍は溜めが長い。
  a.cd=kind.startsWith('dagger')?.29:kind==='spear'?1.02:.84;
}
function attack(a,kind,side){
  let range=kind==='spear'?205:kind.startsWith('dagger')?82:104;
  let arc=kind==='spear'?.11:kind.startsWith('dagger')?.92:1.52;
  let windup=kind==='spear'?.34:kind.startsWith('dagger')?.045:.24;
  let active=kind==='spear'?.12:kind.startsWith('dagger')?.13:.16;
  let recovery=kind==='spear'?.32:kind.startsWith('dagger')?.11:.25;
  let total=windup+active+recovery;
  let e={kind:kind==='spear'?'thrust':'swing',weapon:kind,side,x:a.x,y:a.y,a:a.face,range,arc,t:total,max:total,windup,active,recovery,team:a.team,owner:a,resolved:false,parry:kind.startsWith('dagger')};
  a.attackPose=e;
  effects.push(e);
}
...
function attackPhase(e){let elapsed=e.max-e.t;if(elapsed<e.windup)return 'windup';if(elapsed<e.windup+e.active)return 'active';return 'recovery'}
...
function attacksOverlap(e,f){
  ...
  let rr=(e.weapon&&e.weapon.startsWith('dagger')?38:48)+(f.weapon&&f.weapon.startsWith('dagger')?38:48);
...
// 武器だけターゲット方向へ向ける
x.save();x.rotate(a.face);if(a.spearGuard>0)x.rotate((.72-a.spearGuard)*22);let t=TYPES[a.type];drawWeapon.owner=a;drawWeapon(t.r,1,a.shield,a.handAnimR||0);drawWeapon(t.l,-1,a.shield,a.handAnimL||0);drawWeapon.owner=null;x.restore();if(a===controlled()){x.strokeStyle='#fff';x.lineWidth=3;x.beginPath();x.arc(0,2,34,0,Math.PI*2);x.stroke()}x.restore();x.fillStyle='#17382f';x.font='11px sans-serif';x.textAlign='center';x.fillText(TYPES[a.type].name,a.x,a.y+49);x.textAlign='start'}
...
function drawWeapon(k,side,active,anim=0){
 ...
 if(k==='shield'||k==='dualShield'){
   ...
   let col=weaponColor(k),len=k==='spear'?92:k.startsWith('dagger')?48:58;
   let baseY=side*(k.startsWith('dagger')?30:14),tipY=side*(k.startsWith('dagger')?42:20);
   ...
   let owner=drawWeapon.owner,pose=owner&&owner.attackPose&&owner.attackPose.weapon===k?owner.attackPose:null;
   ...
   x.shadowBlur=18;x.shadowColor=col;x.strokeStyle='#6c625b';x.lineWidth=5;x.beginPath();x.moveTo(k==='spear'?-13:7,baseY);x.lineTo(k==='spear'?18:17,baseY);x.stroke();
   x.strokeStyle=col;x.lineWidth=k==='spear'?7:7;x.beginPath();x.moveTo(k==='spear'?16:16,baseY);x.lineTo(len,tipY);x.stroke();
   x.shadowBlur=0;x.strokeStyle='#ffffff';x.lineWidth=2;x.beginPath();x.moveTo(k==='spear'?20:19,baseY);x.lineTo(len-2,tipY);x.stroke();
...
function drawEffect(e){
 ...
 if(e.kind==='swing'||e.kind==='thrust'){
   ...
   let a=e.owner;if(a&&a.alive){let p=1-e.t/e.max;x.translate(a.x,a.y);x.rotate(a.face+p*22);x.globalAlpha=.35;x.strokeStyle=weaponColor('spear');x.shadowBlur=18;x.shadowColor=weaponColor('spear');x.lineWidth=8;x.beginPath();x.arc(0,0,92,0,Math.PI*2);x.stroke();}
 }else{
