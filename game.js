...
const ENEMY_TEAMS=[
 ...
 {rank:3,style:'電光石火',name:'ノーザン・ファング',colors:['#dcecff','#5aa7e8','#233b66'],formation:['rapier','dagger','katana'],tactics:['attack','flank','balanced'],skillsA:['fiveSlash','fiveSlash','spinSlash'],skillsB:['rapierCounter','substitution','katanaCounter']},
 {rank:3,style:'重圧突破',name:'ヴァーミリオン・タイタンズ',colors:['#8f1f24','#e05a32','#f1c36d'],formation:['greatsword','halberd','spear'],tactics:['attack','support','attack'],skillsA:['spinSlash','longDojoArt','doubleThrust'],skillsB:['steadfast','halberdDoubleSweep','whirlwindAdvance']},
 {rank:3,style:'鉄壁反撃',name:'オブシディアン・ガード',colors:['#282b35','#7f91a8','#e6edf4'],formation:['doubleShield','katana','rapier'],tactics:['defense','defense','flank'],skillsA:['shieldCharge','katanaCounter','rapierCounter'],skillsB:['steadfast','spinSlash','fiveSlash']}
];
...
function useSkill(a,slot='A'){
  ...
  if(skill==='spinSlash'){
  ...
  }else if(skill==='gladiatorAdvance'){
    a[cdKey]=5.8;a.shield=true;a.shieldA=a.face;a.cd=Math.max(a.cd,.92);safePush(a,Math.cos(a.face)*54,Math.sin(a.face)*54);
    let sw={kind:'swing',weapon:'shield',skill:true,side:'l',x:a.x,y:a.y,a:a.face,range:105,arc:.72,t:.42,max:.42,windup:.08,active:.12,recovery:.22,delay:0,team:a.team,owner:a,resolved:false,parry:true,recoveryApplied:false,knockback:42};
    let th={kind:'thrust',weapon:'sword',skill:true,side:'r',x:a.x,y:a.y,a:a.face,range:152,arc:.18,t:.54,max:.54,windup:.08,active:.12,recovery:.34,delay:.24,team:a.team,owner:a,resolved:false,parry:true,recoveryApplied:false,knockback:30,specialHit:'闘士の進撃！'};effects.push(sw,th);a.attackPose=th;setTimeout(()=>{if(a)a.shield=false},520);
  }else if(skill==='phalanxDrive'){
    a[cdKey]=6.0;a.spearGuard=.72;a.spearAdvanceT=.46;a.spearAdvanceVX=Math.cos(a.face)*230;a.spearAdvanceVY=Math.sin(a.face)*230;a.cd=Math.max(a.cd,1.02);effects.push({kind:'spearGuard',owner:a,x:a.x,y:a.y,t:.62,max:.62,team:a.team});
    ...
    a[cdKey]=6.0;a.cd=Math.max(a.cd,.72);safePush(a,Math.cos(a.face)*34,Math.sin(a.face)*34);let th={kind:'thrust',weapon:'gauntletAttack',skill:true,guardPierce:true,side:'r',x:a.x,y:a.y,a:a.face,range:78,arc:.28,t:.52,max:.52,windup:.14,active:.12,recovery:.26,delay:.02,team:a.team,owner:a,resolved:false,parry:false,recoveryApplied:false,lunge:22,lungeApplied:false,knockback:46,specialHit:'発勁！'};effects.push(th);a.attackPose=th;
  }else if(skill==='beastStep'){
    // 翼竜由来の汎用技。スキル枠に見合う高性能回避へ強化。
    a[cdKey]=2.8;a.invuln=Math.max(a.invuln,.32);a.stepCd=Math.max(a.stepCd,.26);
    safePush(a,Math.cos(a.face)*118,Math.sin(a.face)*118);effects.push({kind:'beastStep',owner:a,x:a.x,y:a.y,t:.42,max:.42});ui.status.textContent='空蝉ステップ！';
...
function triggerSubstitution(def,atk){
  if(!def||!atk||!def.alive||!atk.alive)return false;
  def.substitutionReady=false;
  def.counterCharges=0;
  def.counterT=0;
  def.daggerGuard=false;
  def.invuln=Math.max(def.invuln,.55);
  atk.stun=Math.max(atk.stun,.24);

  const ox=def.x,oy=def.y;
  const aa=angle(def,atk);
  ...
  // 相手の横からやや背後へ抜ける
  safePush(def,Math.cos(aa+side*Math.PI/2)*86,Math.sin(aa+side*Math.PI/2)*86);
  ...
  for(let i=0;i<2;i++){
    const sw={
      ...
      x:def.x,y:def.y,a:def.face,range:132,arc:1.08,
      t:.44,max:.44,windup:.025,active:.14,recovery:.275,
      delay:.10+i*.16,team:def.team,owner:def,resolved:false,parry:true,
      recoveryApplied:false,lunge:56,lungeApplied:false,
      knockback:i===1?24:12,
      specialHit:i===1?'変わり身斬り！':null,
      retarget:true,retargeted:false
    };
    effects.push(sw);
    if(i===0)def.attackPose=sw;
  }
  def.cd=Math.max(def.cd,.58);
  ui.status.textContent='変わり身成功！ 丸太を残して側面から二連斬り';
  return true;
}
...
function blocked(def,atk,source=null){
  ...
  if(def.counterT>0){
    ...
    if(def.substitutionReady&&def.type==='dagger'){
      return triggerSubstitution(def,atk);
    }
...
function ai(a,dt){
  if(a.boss&&a.bossKind==='mouse'){let foes=actors.filter(z=>z.team!==a.team&&z.alive);let n=foes.sort((u,v)=>dist(a,u)-dist(a,v))[0];if(n){let aa=angle(n,a)+(Math.random()-.5)*.7;move(a,Math.cos(aa)*1.25,Math.sin(aa)*1.25,dt);if(Math.random()<.025){a.face+=Math.PI*(.65+Math.random()*.7)}}return;}
  ...
  // RANK Sなどの上位チームはスキルBも使う。A/Bを連打せず、距離と戦術に合わせて低頻度で選ぶ。
  if(a.team===1&&a.skillIdB&&a.skillIdB!=='none'&&a.skillCdB<=0&&!defending&&!outnumbered&&Math.random()<(tactic==='defense'?.006:.004)){
    const bRange=a.type==='spear'?205:a.type==='halberd'?190:a.type==='greatsword'?170:a.type==='rapier'?160:a.type==='katana'?150:a.type==='dagger'?140:135;
    if(d<bRange){useSkill(a,'B');return}
  }
  // スキルAは通常攻撃より低頻度。武器と戦術に合う距離でだけ使用。
...
function startTournament(rankIndex){
  let pool=teamsForRank(rankIndex);if(pool.length!==3){q('#tournamentMessage').textContent=`${RANKS[rankIndex]?.label||''}の対戦チーム設定が不足しています。`;return}tournament={rankIndex,index:0,results:[],cpuWins:simulateCpuLeague(rankIndex),teams:pool};q('#tournament').classList.add('hidden');enemyTeam=pool[0];startMatch();
}
function nextTournamentStep(){
  ...
  if(!special&&champion){let rid=RANKS[tournament.rankIndex].id;if(!progress.rankChampions.includes(rid))progress.rankChampions.push(rid);if(progress.pendingBoss===null&&!progress.defeatedBosses.includes(tournament.rankIndex)){progress.pendingBoss=tournament.rankIndex;q('#tournamentMessage').textContent+=' フィールドの「修練の地」に強敵が現れました。';}
    ...
    if(tournament.rankIndex===3)q('#tournamentMessage').textContent+=' RANK S制覇！ フィールドの「時空の歪み」が安定し、伝説の武人たちへ挑戦できるようになりました。';saveProgress();
  }
  tournament=null;
}
...
function openElderEvent(){let ov=q('#questEvent');if(!ov)return;mode='menu';q('#questTitle').textContent='道端の「！」';q('#questText').textContent='武術家らしい老人が困っている。「娘の写真を草むらに落としてしまってな……」';q('#questChoices').innerHTML='<button id="questHelp">助ける</button><button id="questIgnore">見なかったことにする</button>';ov.classList.remove('hidden');q('#questHelp').onclick=()=>{ov.classList.add('hidden');q('#questChoices').innerHTML='';ui.status.textContent='ネズミが写真を咥えてる！ 捕まえろ！';setTimeout(startMouseChase,450)};q('#questIgnore').onclick=()=>{ov.classList.add('hidden');elderEvent.active=false;elderEvent.nextRoll=performance.now()+18000;mode='field';ui.status.textContent='今回は見なかったことにした。老人とはまたどこかで会うかもしれない';syncModeButtons()};}
function startMouseChase(){elderEvent.active=false;let b={name:'写真をくわえたネズミ',type:'dagger',skill:'none',reward:null,hp:1,kind:'mouse',colors:['#8d837b','#d8d0c8','#f5e8d5']};beginBoss(b,{source:'elderQuest'});let e=actors.find(a=>a.boss);if(e){e.r=24;e.speedBonus=155;e.species='mouse';e.furOverride='#8d837b'}ui.mode.textContent='CHASE';ui.status.textContent='ネズミに一撃当てろ！ ネズミは攻撃せず逃げ回る';}
...
function weaponWeight(k){
  ...
  if(k==='gauntletAttack'||k==='gauntletGuard')return 0;
...
function drawWeapon(k,side,active,anim=0){
 ...
 if(k==='shield'||k==='dualShield'){
   ...
   const pose=owner&&owner.attackPose&&owner.attackPose.weapon==='gauntletAttack'?owner.attackPose:null;
   const activeSide=pose?.side||null;
   const phase=pose?attackPhase(pose):null;
   const punchOut=pose&&(phase==='windup'||phase==='active')?10:0;
   for(let sy of [-1,1]){
     // side 'r' を画面下側、'l' を画面上側の拳として扱う。
     const thisSide=sy>0?'r':'l';
     const push=thisSide===activeSide?punchOut:0;
     x.save();x.translate(push,0);
